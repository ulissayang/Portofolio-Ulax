...
function Sk({ className = '' }) {
  return <div className={`animate-pulse bg-blue-100/60 rounded ${className}`}/>;
}
...
function CVDownloadButton({ sm = false }) {
  ...
  const go = async (type) => {
    ...
    const html = type === 'creative' ? buildCreativeCV(d, byCat, byCertGroup) : buildAtsCV(d, byCertGroup);
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)}
        className={`flex items-center gap-2 font-bold rounded-xl transition-all bg-violet-600 hover:bg-violet-700 text-white shadow-lg shadow-violet-500/30 ${sm ? 'px-3 py-2 text-xs' : 'px-5 py-2.5 text-sm'}`}>
        <Download size={sm ? 13 : 15}/> {sm ? 'CV' : 'Download CV'}
      </button>
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpen(false)}/>
          <div className="absolute right-0 top-full mt-2 z-50 bg-white rounded-2xl shadow-2xl border border-slate-200 p-3 w-64">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-1">Pilih format CV:</p>
            <div className="space-y-2">
              {[
                ...
                <button key={opt.id} onClick={() => go(opt.id)} disabled={loading !== null}
                  className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border border-slate-100 hover:border-blue-200 hover:bg-blue-50 text-sm font-semibold transition-all disabled:opacity-50 bg-slate-50">
                  ...
                  {loading === opt.id && <Loader2 size={13} className="animate-spin ml-auto text-blue-500"/>}
                </button>
              ))}
            </div>
            <p className="text-xs text-slate-400 mt-2.5 text-center">Tab baru → Ctrl+P → Save as PDF</p>
          </div>
        </>
      )}
    </div>
...
// CV builders (tidak berubah)
function buildCreativeCV(d, byCat, byCertGroup) {
  return `<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"/><title>CV Kreatif - ${d.hero?.full_name}</title><style>@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap');*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:white;color:#1e293b;print-color-adjust:exact;-webkit-print-color-adjust:exact}.page{max-width:210mm;margin:0 auto;min-height:297mm;display:flex;flex-direction:column;background:white}.layout{display:flex;flex:1}.sidebar{width:68mm;background:linear-gradient(160deg,#1e3a5f,#1d4ed8 60%,#0ea5e9);color:white;padding:28px 20px;flex-shrink:0}.main{flex:1;padding:28px 24px}.av{width:80px;height:80px;border-radius:50%;border:3px solid rgba(255,255,255,0.35);margin:0 auto 10px;overflow:hidden;background:#3b82f6;display:flex;align-items:center;justify-content:center;font-size:28px;font-weight:800;color:white}.av img{width:100%;height:100%;object-fit:cover}.nm{font-size:14px;font-weight:700;text-align:center}.tg{font-size:9px;color:rgba(255,255,255,0.65);text-align:center;margin-top:3px}.div{width:36px;height:2px;background:rgba(255,255,255,0.3);margin:14px auto}.ss{margin-bottom:16px}.st{font-size:8px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:rgba(255,255,255,0.55);margin-bottom:6px}.ci{display:flex;align-items:center;gap:6px;font-size:9px;color:rgba(255,255,255,0.85);margin-bottom:4px;word-break:break-all}.sk{display:inline-block;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.2);color:rgba(255,255,255,0.9);font-size:8px;font-weight:600;padding:2px 7px;border-radius:20px;margin:2px 2px 2px 0}.sc{font-size:7px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.45);margin:7px 0 3px}.mn{font-size:22px;font-weight:800;color:#1e3a5f}.mt{font-size:11px;font-weight:500;color:#3b82f6;margin:3px 0 16px}.ab{font-size:9.5px;color:#475569;line-height:1.7;margin-bottom:18px;border-left:3px solid #3b82f6;padding-left:10px}.sec{margin-bottom:18px}.sh{display:flex;align-items:center;gap:7px;margin-bottom:10px;border-bottom:1.5px solid #e2e8f0;padding-bottom:5px}.si{width:20px;height:20px;background:linear-gradient(135deg,#1d4ed8,#0ea5e9);border-radius:5px;display:flex;align-items:center;justify-content:center;color:white;font-size:10px;flex-shrink:0}.stitle{font-size:11px;font-weight:700;color:#1e3a5f;text-transform:uppercase;letter-spacing:0.5px}.ei{margin-bottom:12px;padding-left:10px;border-left:2px solid #e2e8f0;position:relative}.ei::before{content:'';position:absolute;left:-4px;top:3px;width:7px;height:7px;border-radius:50%;background:#3b82f6;border:1.5px solid white;box-shadow:0 0 0 1px #3b82f6}.et{font-size:10px;font-weight:700;color:#1e293b}.ec{font-size:9px;font-weight:600;color:#3b82f6}.ep{font-size:8px;color:#94a3b8;margin:2px 0 3px}.etask{font-size:8.5px;color:#475569;line-height:1.6;display:flex;gap:4px;align-items:flex-start;margin-bottom:1px}.etask::before{content:'▸';color:#3b82f6;flex-shrink:0}.tcol{display:grid;grid-template-columns:1fr 1fr;gap:12px}.pi{background:#f8fafc;border:1px solid #e2e8f0;border-radius:7px;padding:8px;margin-bottom:6px}.pn{font-size:9.5px;font-weight:700;color:#1e293b}.pd{font-size:8px;color:#64748b;margin:2px 0}.ps{display:flex;flex-wrap:wrap;gap:2px;margin-top:3px}.pt{font-size:7px;font-weight:600;background:#dbeafe;color:#1d4ed8;padding:1px 5px;border-radius:3px}.ci2{font-size:8.5px;color:#475569;display:flex;gap:4px;align-items:flex-start;margin-bottom:3px;line-height:1.5}.ci2::before{content:'✓';color:#0ea5e9;font-weight:700;flex-shrink:0}.ft{background:#1e3a5f;color:rgba(255,255,255,0.55);text-align:center;font-size:8px;padding:8px;margin-top:auto}</style></head><body><div class="page"><div class="layout"><div class="sidebar"><div style="text-align:center;margin-bottom:20px"><div class="av">${d.hero?.avatar_url?`<img src="${d.hero.avatar_url}" alt=""/>`:(d.hero?.avatar_initials||'UL')}</div><div class="nm">${d.hero?.full_name||''}</div><div class="tg">${d.hero?.tagline||''}</div></div><div class="div"></div><div class="ss"><div class="st">Kontak</div>${d.hero?.email?`<div class="ci"><span>@</span>${d.hero.email}</div>`:''} ${d.hero?.phone?`<div class="ci"><span>📞</span>${d.hero.phone}</div>`:''} ${d.hero?.location?`<div class="ci"><span>📍</span>${d.hero.location}</div>`:''} ${d.hero?.github_url?`<div class="ci"><span>GH</span>${d.hero.github_url.replace('https://','')}</div>`:''} ${d.hero?.linkedin_url?`<div class="ci"><span>in</span>${d.hero.linkedin_url.replace('https://','')}</div>`:''}</div><div class="ss"><div class="st">Pendidikan</div>${d.edu.map(e=>`<div style="margin-bottom:8px"><div style="font-size:9px;font-weight:700;color:white">${e.degree}</div><div style="font-size:8px;color:rgba(255,255,255,0.7)">${e.institution}</div><div style="font-size:8px;color:rgba(255,255,255,0.5)">${e.period}</div>${e.gpa?`<div style="font-size:8px;font-weight:700;color:#fbbf24">IPK: ${e.gpa}</div>`:''}</div>`).join('')}</div><div class="ss"><div class="st">Keterampilan</div>${Object.entries(byCat).map(([c,ss])=>`<div class="sc">${c}</div>${ss.map(s=>`<span class="sk">${s}</span>`).join('')}`).join('')}</div></div><div class="main"><div class="mn">${d.hero?.full_name||''}</div><div class="mt">${d.hero?.degree||''} · ${d.hero?.tagline||''} ${d.hero?.gpa?`· IPK ${d.hero.gpa}`:''}</div>${d.about?.description?`<div class="ab">${d.about.description}</div>`:''}<div class="sec"><div class="sh"><div class="si">💼</div><div class="stitle">Pengalaman Kerja</div></div>${d.exp.map(e=>`<div class="ei"><div class="et">${e.title}</div><div class="ec">${e.company}</div><div class="ep">${e.period}</div>${e.description?`<div style="font-size:8.5px;color:#64748b;margin-bottom:2px">${e.description}</div>`:''} ${e.tasks?.length?e.tasks.map(t=>`<div class="etask">${t}</div>`).join(''):''}</div>`).join('')}</div>${d.proj.length>0?`<div class="sec"><div class="sh"><div class="si">🚀</div><div class="stitle">Proyek</div></div><div class="tcol">${d.proj.slice(0,4).map(p=>`<div class="pi"><div class="pn">${p.title}</div>${p.description?`<div class="pd">${p.description.substring(0,90)}...</div>`:''} ${p.tech_stack?.length?`<div class="ps">${p.tech_stack.slice(0,3).map(t=>`<span class="pt">${t}</span>`).join('')}</div>`:''} ${p.demo_url?`<div style="font-size:7px;color:#0ea5e9;margin-top:3px">🌐 ${p.demo_url}</div>`:''}</div>`).join('')}</div></div>`:''} ${d.cert.length>0?`<div class="sec"><div class="sh"><div class="si">🏆</div><div class="stitle">Sertifikasi</div></div><div class="tcol">${Object.entries(byCertGroup).map(([g,items])=>`<div><div style="font-size:8px;font-weight:700;color:#475569;text-transform:uppercase;margin-bottom:3px">${g}</div>${items.map(c=>`<div class="ci2">${c.title}${c.date_label?` (${c.date_label})`:''}</div>`).join('')}</div>`).join('')}</div></div>`:''} ${d.about?.motto?`<div style="background:linear-gradient(135deg,#1e3a5f,#1d4ed8);color:white;padding:12px 16px;border-radius:8px;margin-top:6px"><div style="font-size:8px;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.55);margin-bottom:3px">Moto</div><div style="font-size:11px;font-weight:700;font-style:italic">"${d.about.motto}"</div></div>`:''}</div></div><div class="ft">CV ${d.hero?.full_name} · ${new Date().getFullYear()}</div></div><script>window.onload=()=>window.print();</script></body></html>`;
}

function buildAtsCV(d, byCertGroup) {
  const skills = d.skills.map(s => s.name).join(' · ');
  return `<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"/><title>CV ATS - ${d.hero?.full_name}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;background:white;color:#000;font-size:11pt;line-height:1.45;print-color-adjust:exact}.page{max-width:216mm;margin:0 auto;padding:18mm 20mm;min-height:279mm}@media print{.page{padding:14mm 18mm;max-width:100%}}.hd{text-align:center;border-bottom:2px solid #000;padding-bottom:10px;margin-bottom:12px}.nm{font-size:18pt;font-weight:700;text-transform:uppercase;letter-spacing:1px}.tg{font-size:10pt;color:#333;margin-top:2px}.ct{font-size:9.5pt;margin-top:5px;display:flex;justify-content:center;flex-wrap:wrap;gap:3px 16px}.sec{margin-bottom:12px}.st{font-size:10pt;font-weight:700;text-transform:uppercase;letter-spacing:1px;border-bottom:1px solid #000;padding-bottom:2px;margin-bottom:7px}.eh{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1px}.et{font-size:11pt;font-weight:700}.ec{font-size:10pt;font-weight:600;font-style:italic;color:#222}.ep{font-size:10pt;color:#333;white-space:nowrap;margin-left:8px;font-weight:600}.ei{margin-bottom:9px}.bl{margin:2px 0;font-size:10pt;padding-left:14px;position:relative;line-height:1.5}.bl::before{content:'•';position:absolute;left:3px}.di{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:5px}.dg{font-size:11pt;font-weight:700}.di2{font-size:10pt;color:#333}.dr{text-align:right;flex-shrink:0;margin-left:8px}.dp{font-size:10pt}.dgpa{font-size:10.5pt;font-weight:700}.pi{margin-bottom:7px}.pn{font-size:11pt;font-weight:700}.pd{font-size:10pt;color:#333;margin:1px 0}.ps{font-size:9.5pt;color:#444;font-style:italic}.ci{font-size:10pt;padding-left:14px;position:relative;margin-bottom:2px;line-height:1.5}.ci::before{content:'•';position:absolute;left:3px}.sm{font-size:10.5pt;line-height:1.7;text-align:justify}</style></head><body><div class="page"><div class="hd"><div class="nm">${d.hero?.full_name||''}</div><div class="tg">${d.hero?.tagline||''} ${d.hero?.degree?`| ${d.hero.degree}`:''} ${d.hero?.gpa?`| IPK: ${d.hero.gpa}`:''}</div><div class="ct">${d.hero?.email?`<span>${d.hero.email}</span>`:''} ${d.hero?.phone?`<span>${d.hero.phone}</span>`:''} ${d.hero?.location?`<span>${d.hero.location}</span>`:''} ${d.hero?.github_url?`<span>${d.hero.github_url.replace('https://','')}</span>`:''} ${d.hero?.linkedin_url?`<span>${d.hero.linkedin_url.replace('https://','')}</span>`:''}</div></div>${d.about?.description?`<div class="sec"><div class="st">Ringkasan Profesional</div><div class="sm">${d.about.description}</div></div>`:''} ${d.skills.length>0?`<div class="sec"><div class="st">Keterampilan</div><div style="font-size:10.5pt;line-height:1.8">${skills}</div></div>`:''} ${d.exp.length>0?`<div class="sec"><div class="st">Pengalaman Kerja</div>${d.exp.map(e=>`<div class="ei"><div class="eh"><div><div class="et">${e.title}</div><div class="ec">${e.company}</div></div><div class="ep">${e.period}</div></div>${e.description?`<div style="font-size:10pt;color:#333;margin:1px 0">${e.description}</div>`:''} ${e.tasks?.length?e.tasks.map(t=>`<div class="bl">${t}</div>`).join(''):''}</div>`).join('')}</div>`:''} ${d.proj.length>0?`<div class="sec"><div class="st">Proyek</div>${d.proj.map(p=>`<div class="pi"><div class="eh"><div class="pn">${p.title}</div><span style="font-size:9.5pt;color:#555">${p.status||''}</span></div>${p.description?`<div class="pd">${p.description}</div>`:''} ${p.tech_stack?.length?`<div class="ps">Teknologi: ${p.tech_stack.join(', ')}</div>`:''} ${p.demo_url?`<div class="ps">URL: ${p.demo_url}</div>`:''}</div>`).join('')}</div>`:''} ${d.edu.length>0?`<div class="sec"><div class="st">Pendidikan</div>${d.edu.map(e=>`<div class="di"><div><div class="dg">${e.degree}</div><div class="di2">${e.institution}</div></div><div class="dr"><div class="dp">${e.period}</div>${e.gpa?`<div class="dgpa">IPK: ${e.gpa}</div>`:''}</div></div>`).join('')}</div>`:''} ${d.cert.length>0?`<div class="sec"><div class="st">Sertifikasi & Pelatihan</div>${Object.entries(byCertGroup).map(([g,items])=>`<div style="margin-bottom:5px"><div style="font-weight:700;font-size:10.5pt;margin-bottom:2px">${g}</div>${items.map(c=>`<div class="ci">${c.title}${c.institution?` – ${c.institution}`:''} ${c.date_label?`(${c.date_label})`:''}</div>`).join('')}</div>`).join('')}</div>`:''}</div><script>window.onload=()=>window.print();</script></body></html>`;
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [hero, about, exp, ach, proj, edu, cert, skills] = await Promise.all([
        supabase.from('portfolio_hero').select('*').single(),
        supabase.from('portfolio_about').select('*').single(),
        supabase.from('portfolio_experiences').select('*').order('sort_order'),
        supabase.from('portfolio_achievements').select('*').order('sort_order'),
        supabase.from('portfolio_projects').select('*').order('sort_order'),
        supabase.from('portfolio_educations').select('*').order('sort_order'),
        supabase.from('portfolio_certifications').select('*').order('sort_order'),
        supabase.from('portfolio_skills').select('*').order('sort_order'),
  ...
  const navLinks = [
    { id:'home', l:'Beranda' }, { id:'about', l:'Tentang' },
    { id:'experience', l:'Pengalaman' }, { id:'projects', l:'Proyek' },
    { id:'achievements', l:'Pencapaian' }, { id:'education', l:'Pendidikan' },
    { id:'keterampilan', l:'Keterampilan' },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-2">
              {hero?.github_url && (
                <a href={safeUrl(hero.github_url)||'#'} target="_blank" rel="noreferrer"
              ...
              {hero?.linkedin_url && (
                <a href={safeUrl(hero.linkedin_url)||'#'} target="_blank" rel="noreferrer"
                  className="w-9 h-9 rounded-xl bg-blue-50 hover:bg-blue-100 flex items-center justify-center text-blue-600 transition-all">
                  <Linkedin size={16}/>
                </a>
              )}
              <CVDownloadButton sm/>
            ...
            {/* Hamburger */}
            <button className="lg:hidden w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center text-slate-600"
              ...
              {navLinks.map(l => (
                <button key={l.id} onClick={() => scrollTo(l.id)}
                  className={`block w-full text-left px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
                    active === l.id ? 'bg-blue-50 text-blue-600 font-semibold' : 'text-slate-700 hover:bg-slate-50'
                  }`}>{l.l}
                </button>
              ))}
              <div className="flex items-center gap-2 px-3 pt-3 pb-1 border-t border-slate-100 mt-1">
                {hero?.github_url && <a href={safeUrl(hero.github_url)||'#'} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm text-slate-600 font-medium"><Github size={15}/> GitHub</a>}
                {hero?.linkedin_url && <a href={safeUrl(hero.linkedin_url)||'#'} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm text-blue-600 font-medium"><Linkedin size={15}/> LinkedIn</a>}
      ...
      {/* ══ HERO ══ */}
      <section id="home" className="pt-16 pb-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-14">
        <div className="flex-1 text-center md:text-left space-y-6">
          {loading ? (
            <><Sk className="h-6 w-44 mx-auto md:mx-0"/><Sk className="h-14 w-full max-w-md mt-3"/><Sk className="h-5 w-full max-w-lg mt-2"/><Sk className="h-5 w-3/4 max-w-lg mt-1"/></>
          ) : (
            <>
              ...
              <div className="flex flex-wrap gap-3 justify-center md:justify-start pt-1">
                <button onClick={() => scrollTo('projects')}
                  className="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm flex items-center gap-2 shadow-lg shadow-blue-500/30 transition-all hover:-translate-y-0.5">
                  <Layers size={16}/> Lihat Proyek
                </button>
                <a href={`mailto:${hero?.email}`}
                  className="px-6 py-3 rounded-xl bg-white hover:bg-slate-50 text-slate-700 font-semibold text-sm flex items-center gap-2 border border-slate-200 shadow-sm transition-all hover:-translate-y-0.5">
                  <Mail size={16}/> Hubungi Saya
                </a>
                <CVDownloadButton/>
              </div>
              <div className="flex flex-wrap items-center gap-5 pt-2 text-sm text-slate-500 justify-center md:justify-start">
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8 flex flex-col items-center text-center hover:scale-[1.02] transition-transform duration-300 relative overflow-hidden">
              ...
              <div className="w-32 h-32 rounded-full mb-5 border-4 border-white shadow-xl relative overflow-hidden bg-gradient-to-br from-blue-100 to-indigo-100 z-10">
                {loading ? <Sk className="w-full h-full rounded-full"/> : hero?.avatar_url
                  ? <img src={hero.avatar_url} alt={hero?.full_name} className="w-full h-full object-cover"/>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {hero?.gpa && (
                    <div className="mt-4 px-5 py-2 rounded-full bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 z-10">
                      <p className="text-blue-700 font-bold text-sm">IPK: {hero.gpa}</p>
                    </div>
                  )}
                </>
              )}
      ...
      <section id="about" className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 rounded-2xl bg-blue-100 flex items-center justify-center"><User size={20} className="text-blue-600"/></div>
            <h2 className="text-3xl font-extrabold text-slate-900">Tentang Saya</h2>
          </div>
          <div className="bg-slate-50 rounded-2xl border border-slate-100 p-8 shadow-sm space-y-6">
            {loading ? <><Sk className="h-5 w-full"/><Sk className="h-5 w-5/6"/><Sk className="h-5 w-4/6"/></> : (
              <>
                ...
                {about?.motto && (
                  <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-7 rounded-2xl relative overflow-hidden shadow-xl shadow-blue-500/20">
                    ...
                    <p className="text-blue-200 text-xs font-bold uppercase tracking-widest mb-3 flex items-center gap-2">
                      <Award size={13}/> Moto Profesional
                    </p>
      ...
      <section id="experience" className="py-20 bg-slate-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-14">
            <div className="w-10 h-10 rounded-2xl bg-blue-100 flex items-center justify-center"><Briefcase size={20} className="text-blue-600"/></div>
            <h2 className="text-3xl font-extrabold text-slate-900">Pengalaman Kerja</h2>
          </div>
          <div className="relative">
            {/* Timeline line */}
            ...
            <div className="space-y-10">
              {loading ? [1,2,3].map(i => (
                <div key={i} className="flex gap-5 items-start pl-14 md:pl-0">
                  <Sk className="w-10 h-10 rounded-full shrink-0 hidden md:block"/>
                  <Sk className="flex-1 h-44 rounded-2xl"/>
                </div>
              )) : data.experiences.map((exp, idx) => (
                <div key={exp.id} className={`relative flex md:items-center ${idx % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-8`}>
                  {/* Dot */}
                  <div className="absolute left-5 md:left-1/2 w-10 h-10 rounded-full border-4 border-white shadow-lg flex items-center justify-center text-white -translate-x-1/2 z-10 shrink-0"
                      ...
                      <ul className="space-y-2">
                        {exp.tasks.map((t,i) => (
                          <li key={i} className="flex gap-2 text-sm text-slate-600 items-start">
                            <ChevronRight size={13} className="shrink-0 mt-0.5" style={{color:exp.color||'#3b82f6'}}/>
                            {t}
                          </li>
                        ))}
                      </ul>
      ...
      <section id="projects" className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-2xl bg-teal-100 flex items-center justify-center"><Layers size={20} className="text-teal-600"/></div>
            <h2 className="text-3xl font-extrabold text-slate-900">Proyek</h2>
          </div>
          <p className="text-slate-500 text-lg mb-12 ml-14">Sistem dan aplikasi yang telah dibangun dan digunakan secara nyata.</p>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-slate-50 rounded-2xl border border-slate-200">
              ...
              <p className="text-slate-400 font-medium">Proyek belum ditambahkan</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {data.projects.map(proj => (
                <div key={proj.id} className="group bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 overflow-hidden flex flex-col">
                  {/* Screenshot */}
                  <div className="h-44 overflow-hidden relative bg-gradient-to-br from-slate-100 to-blue-50">
                    {proj.image_url
                      ...
                      : <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                          ...
                          <span className="text-xs text-slate-400 font-medium">Tidak ada screenshot</span>
                        </div>
                    }
                    {proj.status && (
                      <span className={`absolute top-3 right-3 text-xs font-bold px-2.5 py-1 rounded-full text-white shadow ${
                        proj.status==='Aktif' ? 'bg-emerald-500' : proj.status==='Selesai' ? 'bg-blue-500' : 'bg-slate-500'
                      }`}>{proj.status}</span>
                    )}
                  </div>
                  {/* Content */}
                    ...
                    {proj.tech_stack?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {proj.tech_stack.map((t,i) => (
                          <span key={i} className="text-xs font-semibold bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md border border-slate-200">{t}</span>
                        ))}
                      </div>
                    )}
                    <div className="flex gap-2 mt-4 pt-3 border-t border-slate-100">
                      {proj.demo_url && (
                        <a href={safeUrl(proj.demo_url)||'#'} target="_blank" rel="noreferrer"
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors">
                          <Globe size={13}/> Demo
                        </a>
                      )}
                      {proj.github_url && (
                        <a href={safeUrl(proj.github_url)||'#'} target="_blank" rel="noreferrer"
                          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-900 text-white text-sm font-bold transition-colors">
      ...
      <section id="achievements" className="py-20 bg-slate-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-14">
            <div className="w-10 h-10 rounded-2xl bg-amber-100 flex items-center justify-center"><Award size={20} className="text-amber-600"/></div>
            <h2 className="text-3xl font-extrabold text-slate-900">Pencapaian</h2>
          </div>
          {loading ? <Sk className="h-56 w-full rounded-2xl"/> : data.achievements.map(ach => (
            <div key={ach.id} className="mb-8 bg-gradient-to-br from-amber-50 via-orange-50 to-amber-50 border border-amber-200 rounded-3xl p-8 md:p-10 shadow-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                {ach.demo_url && (
                  <a href={safeUrl(ach.demo_url)||'#'} target="_blank" rel="noreferrer"
                    className="inline-flex items-center gap-2 mb-6 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm shadow-lg shadow-blue-500/25 transition-all hover:-translate-y-0.5">
                    <Globe size={14}/> Lihat Demo / Website
                  </a>
                )}
                {ach.project_title && (
                  <div className="bg-white rounded-2xl p-6 border border-amber-100 shadow-md max-w-3xl">
                    ...
                    {ach.tags?.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {ach.tags.map((t,i) => <span key={i} className="px-3 py-1 text-xs font-bold bg-slate-100 text-slate-700 rounded-lg border border-slate-200">{t}</span>)}
                      </div>
                    )}
                  </div>
                )}
      ...
      <section id="education" className="py-20 bg-white border-t border-slate-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-16">
            ...
            <div>
              <div className="flex items-center gap-3 mb-8">
                <div className="w-10 h-10 rounded-2xl bg-blue-100 flex items-center justify-center"><GraduationCap size={20} className="text-blue-600"/></div>
                <h2 className="text-3xl font-extrabold text-slate-900">Pendidikan</h2>
              </div>
              <div className="space-y-4">
                {loading ? [1,2].map(i => <Sk key={i} className="h-32 rounded-2xl"/>) :
                  data.educations.map(edu => (
                    <div key={edu.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow p-6 relative overflow-hidden">
            ...
            <div>
              <div className="flex items-center gap-3 mb-8">
                <div className="w-10 h-10 rounded-2xl bg-teal-100 flex items-center justify-center"><Award size={20} className="text-teal-600"/></div>
                <h2 className="text-3xl font-extrabold text-slate-900">Sertifikasi & Pelatihan</h2>
              </div>
              <div className="space-y-4">
                {loading ? [1,2].map(i => <Sk key={i} className="h-40 rounded-2xl"/>) :
                  Object.entries(certGroups).map(([group, items]) => (
                    <div key={group} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                      <h3 className="font-bold text-base text-slate-800 border-b border-slate-100 pb-3 mb-4">{group}</h3>
                      <div className="space-y-3">
                        {items.map(cert => (
                          <div key={cert.id} className="flex gap-3 items-start group">
                            <div className={`p-1.5 rounded-lg shrink-0 mt-0.5 ${cert.type==='certification' ? 'bg-teal-100 text-teal-600' : 'bg-blue-100 text-blue-600'}`}>
                              <CheckCircle2 size={14}/>
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="font-semibold text-sm text-slate-800">{cert.title}</p>
                              <p className="text-xs text-slate-400 mt-0.5">
                                {cert.institution && <span className="text-slate-500">{cert.institution} · </span>}
                                {cert.date_label}
                              </p>
                              {cert.certificate_url && (
                                <a href={safeUrl(cert.certificate_url)||'#'} target="_blank" rel="noreferrer"
                                  className="inline-flex items-center gap-1 mt-1.5 text-xs font-bold text-teal-600 hover:text-teal-700 bg-teal-50 hover:bg-teal-100 px-2 py-0.5 rounded-md transition-colors">
                                  <ExternalLink size={10}/> Lihat Sertifikat
                                </a>
                              )}
                            </div>
                          </div>
      ...
      {/* ══ SKILLS ══ */}
      <section id="keterampilan" className="py-20 bg-gradient-to-br from-slate-50 to-blue-50 border-t border-slate-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            <div className="grid md:grid-cols-3 gap-5">{[1,2,3].map(i => <Sk key={i} className="h-36 rounded-2xl"/>)}</div>
          ) : Object.keys(skillGroups).length > 0 ? (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {Object.entries(skillGroups).map(([cat, skills]) => (
                <div key={cat} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 hover:shadow-md transition-shadow">
            ...
            <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
              {data.skills.map(skill => (
                <span key={skill.id} className="flex items-center gap-2 px-5 py-3 bg-white rounded-xl border border-slate-200 text-slate-700 font-bold shadow-sm hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 hover:-translate-y-0.5 transition-all cursor-default">
      ...
      <section className="py-20 bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a href={`mailto:${hero?.email}`}
              className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-white text-blue-700 font-bold text-base hover:bg-blue-50 hover:scale-105 transition-all shadow-xl">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            <p className="font-extrabold text-xl text-white">
              <span className="text-blue-400">{hero?.brand_name||'ULIS.'}</span>
              {hero?.brand_suffix||'PORTFOLIO'}
            </p>
